...
data class CountedReservation(val originalReservation: ReservedInstances, val location: Location, var unmatchedCount: Int = originalReservation.instanceCount()): AWSResource() {
    val state: String = originalReservation.stateAsString()
    val isActive: Boolean = state == "active"
    val availabilityZone: String? = originalReservation.availabilityZone()
    val scope: String? = originalReservation.scopeAsString()
    val instanceType: String = originalReservation.instanceTypeAsString()
    val family: String = originalReservation.instanceTypeAsString().split(".")[0]
    val instanceSize: String = originalReservation.instanceTypeAsString().split(".")[1]
    ...
    val productDescription: String = originalReservation.productDescriptionAsString()
    val end: Date? = Date.from(originalReservation.end())
...
data class MatchedInstance(val originalInstance: Instance, val location: Location, var matched: Boolean = false): AWSResource() {
    ...
    val isRunning: Boolean = this.state.nameAsString() == "running"
    val availabilityZone: String = originalInstance.placement().availabilityZone()
    val instanceType: String = originalInstance.instanceTypeAsString()
    val family: String = originalInstance.instanceTypeAsString().split(".")[0]
    val instanceSize: String = originalInstance.instanceTypeAsString().split(".")[1]
    val computeUnits: Float = computeUnitsFor(instanceSize)
    val platform: String = if(originalInstance.platformAsString().equals("windows", ignoreCase = true)) "Windows" else "Linux/UNIX"
    val vpcId: String? = originalInstance.vpcId()
    val isVpc: Boolean = !vpcId.isNullOrEmpty()
    val launchDate: String? = DateTimeFormatter.ISO_DATE.format(originalInstance.launchTime().atZone(ZoneId.of("UTC")).toLocalDateTime())
    ...
    fun sameZoneAs(reservation: CountedReservation) = (reservation.availabilityZone == this.availabilityZone) ||
            (reservation.scope == "Region" && this.availabilityZone.startsWith(reservation.location.region.toString()))
    ...
    fun isNotRunning() = state.nameAsString() != "running"
}
...
data class ApplicationLoadBalancer(val originalLoadBalancer: LoadBalancer,
                                   val listeners: List<software.amazon.awssdk.services.elasticloadbalancingv2.model.Listener>,
...
data class RDSInstance(val originalInstance: DBInstance, val location: Location): AWSResource() {
    ...
    val multiAZ: Boolean = originalInstance.multiAZ()
...
data class DomainName(val originalInstance: ResourceRecordSet): AWSResource() {
    ...
    val type: String = originalInstance.typeAsString()
...
data class EBSVolume(val originalInstance: Volume, val location: Location): AWSResource() {
    ...
    val state = originalInstance.stateAsString()
    val type = originalInstance.volumeTypeAsString()
...
data class VPCSubnet(val originalInstance: Subnet, val location: Location): AWSResource() {
    ...
    val default = originalInstance.defaultForAz()
}

data class CFStack(val originalInstance: software.amazon.awssdk.services.cloudformation.model.Stack, val location: Location): AWSResource() {
...
data class TargetGroup(val originalInstance: software.amazon.awssdk.services.elasticloadbalancingv2.model.TargetGroup): AWSResource() {
...
open class AdvisorResult(val check: Check, val region: String, val resourceType: String, val resourceId: String, val description: String, val saving: String = "", val rating: String = "None") {
    companion object {
        fun create(check: Check, rr: DescribeTrustedAdvisorCheckResultResponse?): List<AdvisorResult> {
            rr ?: return emptyList()
